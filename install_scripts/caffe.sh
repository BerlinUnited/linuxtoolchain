...
if [ "$1" = "check" ]; then
  ...
  cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$EXTERN_DIR -DCMAKE_PREFIX_PATH=$EXTERN_DIR -DCPU_ONLY=ON -DUSE_LEVELDB=OFF -DUSE_LMDB=OFF -DBUILD_python=OFF -DUSE_OPENCV=OFF -DBUILD_docs=OFF ..
  make && make install
